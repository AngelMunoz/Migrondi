...
type LabeledField =
  ...
  static member inline Horizontal
    (label: string, content: Control, ?spacing: int)
    =
    StackPanel()
      .Classes("LabeledField_Horizontal")
      .OrientationHorizontal()
...
module ProjectDetails =
        ...
        StackPanel()
          ...
          .Children(
            ...
            (match error with
             ...
             | :? SourceNotFound as me ->
               sourceNotFoundDisplay(me.path, me.name)
  ...
  type MigrationsPanel
    ...
    let dryRunView =
      dryRunView
      |> Option.defaultWith(fun () -> failwith "DryRunView is not set")
