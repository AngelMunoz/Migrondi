...
module Options =
    ...
    type ListOptions =
        { [<Option('a', "all", Required = false, HelpText = "Shows every migration ran against the database.", Default = false)>]
          all: bool
          [<Option('m', "missing", Required = false, HelpText = "Shows the migrations that are pending to run.", Default = false)>]
          missing: bool
          [<Option('l', "last", Required = false, HelpText = "Shows the last migration run agains the database.", Default = true)>]
          last: bool }
