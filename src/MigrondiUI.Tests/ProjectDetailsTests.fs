...
let ``DryRunView - wraps with transaction when manualTransaction is false`` () =
    ...
    Assert.EndsWith("-- ----------COMMIT TRANSACTION----------", textEditor.Text.TrimEnd([|'\n';'\r'|]))
...
let ``migrationListView - shows migration views for each migration`` () =
    let migration1 = { name = "test1"; timestamp = 0L; upContent = "up1"; downContent = "down1"; manualTransaction = false }
    let migration2 = { name = "test2"; timestamp = 1L; upContent = "up2"; downContent = "down2"; manualTransaction = false }
    let statuses = [| Applied migration1; Pending migration2 |]
    let view = migrationListView statuses
    let itemsControl = view :?> ItemsControl
    let items = [| for item in itemsControl.ItemsSource -> item :?> MigrationStatus |]
...
let ``dryRunListView - shows dry run views for each migration`` () =
    ...
    Assert.Equal(2, (itemsControl.ItemsSource :?> (Migration * RunMigrationKind)[]).Length)
    let footer = stackPanel.Children[2] :?> TextBlock
    Assert.Equal("Simulation completed. No changes were made to the database.", footer.Text)
